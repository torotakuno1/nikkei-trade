# NIKKEI TRADE 統合システム - 引き継ぎ v12

## ⚠️ Claude運用ルール

- **Pine Script → チャット本文にコードブロック**（TVコピペ用。アーティファクト不可）
- **Python → ファイル出力**（Mac/PCで実行するため）

---

## ドキュメント構成

本ファイル = 意思決定・結論・教訓のみ。コード・詳細データは別ファイル参照。
- scripts/pine/ → v6, 案C Pine Script
- scripts/execution/ → v6, 案C, Gold EWMAC リアルタイムエンジン（**Webhook方式に移行予定**）
- scripts/data/ → build_nk225_database_v3.py（Panama Canal adjustment付き。ただしWFA用途ではTV CSVが正）
- research/ → regime_analysis_results.csv, wfa_results/
- data/README.md → データファイル所在

---

## システム構成と成績

### v6 — 日経225 1H トレンドフォロー（主力）

イン: NTサイン(MACD12/26/9, RSI14, DI14/ADX20閾値20, KVO34/55, スコア≥2) + VM(StdDev26, BB21σ0.6)同方向 + MTF(1H EMA20+MACD)同方向 + D20MA方向一致 + D20MA傾き≥0.05% + LDN16除外 + SQ週除外
アウト: NT反転 | SL 300円 | VT20 Max5枚

成績(2020/1-2026/3, TV strategy tester): NP 894.2万 | DD 72.6万 | RF 12.23 | 180trades
全期間(2013-2026, TV 360trades/1枚): NP +146.9万 | PF 1.50 | RF 2.96

### 案C — 日経225 1H ハイブリッド（補完）

イン: (VM新規+NTスコア) OR (NTフル+VM背景) + D20MA方向 + LDN16除外 + VR>1.00
アウト: VM消灯 OR NT反転 | SL 300円 | VT1+DD固定2枚

成績(同期間, TV strategy tester): NP 509.0万 | DD 57.7万 | RF 8.77 | 726trades
全期間(2008-2026, TV 1809trades/1枚): NP +294.7万 | PF 1.27 | RF 5.02

### Gold EWMAC — 2H トレンドフォロー

シグナル: EWMAC barbell(8,32)+(64,256) ロングのみ
フィルター: GVZ zscore > 0
VT20% Max5, IDM=1.2, 慣性10% | 商品: MGC or GC

成績(2019/1-2026/4, 7年超): NP 977.8万 | DD 57.1万 | RF 17.14 | Active 42%

### USD/JPY — 保留

---

## v6/案C ウォークフォワード分析（WFA）— TV トレードリスト直接分析

**方法:** Pine Script strategy testerのトレードリストCSVを直接IS/OOS分割。Pythonシグナルエンジンは使用しない（理由は教訓#50参照）。
**設定:** IS=18mo, OOS=6mo, step=6mo。パラメータ最適化なし（固定パラメータのOOS損益のみ評価）。1枚あたりに正規化。

### v6 WFA結果

| 期間 | 窓数 | OOS黒字率 | 平均PF | NP/1枚 | PF | RF |
|------|------|-----------|--------|--------|-----|-----|
| 全期間 2013-2026 | 22 | 14/22 (**64%**) | 2.58 | +146.9万 | 1.50 | 2.96 |
| 前半 2013-2017 | 8 | 2/8 (25%) | — | -45.1万 | 0.57 | -1.01 |
| **後半 2018-2026** | **13** | **10/13 (77%)** | **3.63** | **+192.1万** | **2.02** | **5.26** |

**解釈:** v6は2018年以降の市場構造でロバスト（77%黒字）。2013-2017はPF 0.57で構造的にマイナス。v6のエッジは全時代普遍ではなく、2018年以降の市場構造（高ボラ・トレンド発生頻度）に依存。

### 案C WFA結果

| 期間 | 窓数 | OOS黒字率 | 平均PF | NP/1枚 | PF | RF |
|------|------|-----------|--------|--------|-----|-----|
| 全期間 2008-2026 | 33 | 26/33 (**79%**) | 1.29 | +295.9万 | 1.28 | 5.04 |
| **2018年以降** | **14** | **13/14 (93%)** | **1.42** | **+294.3万** | **1.43** | **7.10** |

**解釈:** 案Cは全期間でも79%黒字。2018以降は93%で圧倒的。OR条件の高頻度トレード（N=946）が分散効果を生み、時代依存性がv6より大幅に低い。

### v6 vs 案C 比較（2018年以降）

| 指標 | v6 | 案C | 優位 |
|------|-----|-----|------|
| OOS黒字率 | 77% | **93%** | 案C |
| PF | **2.02** | 1.43 | v6 |
| RF | 5.26 | **7.10** | 案C |
| NP/1枚 | +192万 | **+294万** | 案C |
| N | 222 | **946** | 案C |

**結論:** AND条件厳選型(v6)はPF高いが時代依存強い。OR条件高頻度型(案C)はPFは低いがロバスト性圧倒的。2システム並行運用の設計判断は正しい。

---

## ★ 方針転換: v6/案C ライブ執行を TV Webhook 方式に変更

### 経緯

v6 WFA 13年をPythonシグナルエンジンで実行したところ、全23窓IS/OOS全マイナス（PF 0.05）。原因調査の結果、**Pine ScriptとPythonのシグナル生成が構造的に一致しない**ことが判明。

TV トレードリスト360件とPythonシグナルを突合せた結果:
- Pythonが検出できたのは**23%のみ**（77%が不一致）
- 不一致の主因: EMA初期値差異、BB帯幅のddof、intrabar path simulation、request.security()再現困難性など**最低6層の構造的差異**
- 限月調整（Panama Canal backward adjustment）を実装しても改善なし（相対指標には影響しないため）
- 業界コンセンサス: 「Pine→Python 100%一致は構造的に不可能」（QuantConnect公式見解）

### 新方式

```
TV Pine Script (v6/案C)
  ↓ Webhook (HTTP POST, alert発火時)
Python サーバー (miniPC, Flask/FastAPI)
  ↓ パース → 発注ロジック
IBKR IB Gateway (ブラケットオーダー)
  ↓
Telegram通知
```

**メリット:**
- Pine Scriptがシグナル源 → 一致率100%（PythonはIBKR発注のみ担当）
- 現行のブラケット注文・再接続・Telegram通知コードは流用可能
- Pythonシグナルエンジン部分が不要になりコードが大幅簡素化

**要件:**
- TradingView Premium（契約済み）
- miniPCでWebhookサーバー常時稼働
- レイテンシー: 1-5秒（1H足戦略には十分）

### Gold EWMACの扱い

Gold EWMACはPythonファーストで設計されたシステムであり、Pine Script依存がない。**現行のPythonエンジン（gold_ewmac_engine.py）をそのまま継続**。Webhook化の対象外。

---

## Phase 状態

- Phase 0-4: ✅完了
- Phase 5-6: ✅完了（3エンジン稼働中。ただしv6/案Cは**Webhook方式に移行予定**）
- **Phase 7: 🔄 ペーパートレード中** → Webhook移行後に再開
- Phase 8: 未着手（ライブ）

**次のマイルストーン:** v6/案C Webhook実装 → ペーパー並行稼働 → ライブ

---

## 教訓（v12追加分 #50-53）

過去の教訓#1-49はv8/v10/v11参照。

50. **Pine Script→Python完全一致は構造的に不可能。** EMA初期値(first value vs SMA seed)、intrabar path simulation、request.security()のlookahead、BB帯幅のddof(N vs N-1で2.6%差)、連続先物構築方式、取引所セッション定義の少なくとも6層で差異が発生。QuantConnect公式: 「100%一致は通常不可能であり目的にすべきでない」。PyneCore(14-15桁精度)で指標計算は解決可能だが、データ層の差異は解決不能。

51. **限月調整(Panama Canal backward adjustment)は相対指標(MACD/RSI/ADX/BB/EMA)に影響しない。** 各限月期間内ではオフセットが一定であり、差分・比率ベースの指標は不変。build_nk225_database_v3.pyで実装・検証済み。v6 WFA結果はv2(無調整)とv3(調整済み)で同一。教訓#48の「影響軽微」判断は正しかった。

52. **AND条件厳選型(v6)はPF高いが時代依存が強い。OR条件高頻度型(案C)はPF低いがロバスト性が圧倒的。** v6: 2013-2017 PF 0.57（構造的マイナス）、2018-2026 PF 2.02。案C: 全期間OOS黒字79%、2018以降93%。高頻度トレードの分散効果が時代依存性を緩和する。

53. **Pine Scriptで開発したシステムのライブ執行は、Webhook中継が最も合理的。** Pythonでのロジック再実装は6層の構造的差異により一致率23%（v6突合せ実績）。Webhook方式なら一致率100%でレイテンシー1-5秒（1H足戦略に十分）。Pythonファーストで設計したシステム(Gold EWMAC)にはこの問題は発生しない。

54. **次元独立性: 同一次元の指標複数（RSI+DI差分=モメンタム×2）はRF 1.40。多次元独立フィルター（トレンド×モメンタム×ボラ×MTF×時間）はRF 12.17。** 新システム設計時、採用指標の次元分類を最初に行い、同一次元2つ以上は冗長と判断すべき。

55. **エグジット遅延は最も危険な変更。** NT反転エグジットに「ADX高い間ホールド」等の遅延ロジック追加 → RF 12.26→1.91〜7.56。エグジットは早すぎる方が安全。

56. **VM条件は再エントリーで迂回不可。** VM再点灯待ちだと0回発火、VM不要にするとRF悪化。

57. **最適化済みAND条件は追加も削除も不可。** A-Kプリセット全滅（全てRF 12.17以下）。条件緩和も条件追加も改善しない。

58. **トレンド指標は「水準」より「位相」（速度・加速度）で使え。** ADX≥25等のレベルフィルターより、ADX velocity（1次微分）の方がfeature importance 1位。Gold EWMACのGVZ zscore>0も同じ原則。

59. **位相ラベルの離散化（芽生え/成長/成熟/衰退の4値）はfeature importance最下位。** 連続値（velocity, acceleration）のまま使う方が情報量を保持する。

60. **NTスコア+VM構造はN225固有のエッジ。** ESではv6 RF 0.86（NP全体の74.5%が1トレード依存）、案C RF 0.31（PF 1.02、781トレードでほぼランダム）。商品拡大にはパラメータフリーのEWMAC方式が適する。

61. **ゴールドへのキャリー追加は無効。** コンタンゴ88%の商品ではキャリーFCが常にショート方向（平均-12.1）→ long only EWMACと逆行。EWMAC60+Carry40 LO: RF 3.10 vs ベースラインRF 6.78。キャリーが有効なのは金利先物など期間構造が変動する商品。

62. **EWMACへのADXフィルター追加は無効（7パターン全滅）。** EWMACのフォーキャスト値自体がトレンド強度を内包しており、ADXは同一次元の冗長情報。最良でもΔRF -0.39。教訓#54の再確認。

63. **4年データの商品スクリーニングは統計的に不十分。** pysystemtradeの長期データ（数十年）で結論が逆転する商品がある（MCL: 4年SR -0.55 → 長期SR 0.465）。商品選定は最低10年以上のデータで判断すべき。

64. **フェード2HシステムのPythonバックテストはTVと大幅乖離する。** Python RF 500超 vs TV RF 1.41。2Hバー構築の構造的不一致（pd.resample vs TVセッション境界対応）が原因。教訓#50と同じ構造。フェード2Hの全検証はTV strategy tester + xlsxエクスポートで実施すること。

65. **フェードシステムにSLは逆効果。** SL 1000pt発動7件中6件でSLの方が損失拡大（合計-88万→-140万）。深い逆行を耐えて戻るのがこの戦略の構造。保護はSLではなくモメンタム反転検出で行う。

66. **エントリーフィルターの裏返しがエグジットに最適。** MACD+KVOブルクロスでエントリー → ベアクロスでエグジット。RF 3.23→6.05（+87%）、DD 65万→43万（-34%）。エントリー根拠の消失＝撤退は論理的に一貫。

67. **保護エグジットの最適時間足はエントリーと同一（1H）。** 下位足（30m/45m）はノイズで早切り、上位足（90m/2H）は反応遅延。5TF×3条件を網羅テスト済み。

---

## 未テスト・将来の検討事項

**最優先:**
- pysystemtradeでレジーム別SR（直近5年 vs 長期）を商品別に出す → 商品選定確定
- 選定した商品のEWMACエンジン実装（gold_ewmac_engine.pyフレームワーク流用）

**優先度高:**
- pysystemtrade導入済み（WS07: C:\Users\CH07\pysystemtrade）。253商品の長期データ利用可能
- 商品候補の長期SR: US10 0.533, CRUDE_W 0.465, NASDAQ 0.361, EUR 0.296, GOLD 0.246, SP500 0.110
- FirstRate Complete Intraday購入 → レジーム調査 + 5分足アルゴ商品選定
- Carverの『Advanced Futures Trading Strategies』読了
- 商品数拡大検討（MES/MCL等マイクロ先物）

**優先度中:**
- Gold EWMACのトレードリスト作成 → レジーム分析
- USD/JPY次トレンド時に再開
- v6の2013-2017マイナス期のレジーム特定（既存VIX/USDJPY日足データで実施可能）

**保留/棄却:**
- Pythonシグナルエンジンの一致率改善 → Webhook方式で不要に
- PyneCore導入 → 将来の新システムがPine依存する場合のみ検討

---

## 口座・商品情報

口座: IBKR Japan | 純資産: 300万（2026/4増資済み）
商品: NK225MC(OSE.JPN, ×10) | MGC(COMEX, 取引可確認済)
miniPC: Getorli Ryzen 5300U/16GB/Win11Pro | IB Gateway port 4002(paper)/4001(live)
リモート: Tailscale + Parsec

---

## インフラ構成（v9と同一。Webhook追加予定）

（詳細はv9参照。v12での変更: v6/案Cエンジンを Webhook受信サーバーに置換予定）

---

## テスト完了サマリー

N225: 約100パターン（エントリー90+ / ポジションサイジング13）
Gold EWMAC: 約4,500+パターン
N225レジーム×マクロ: 9指標×複数ビン
v6 WFA: TV直接分析22窓（全期間64%、2018以降77%黒字）
案C WFA: TV直接分析33窓（全期間79%、2018以降93%黒字）
Pine→Python一致検証: 突合せ実施（一致率23%、構造的不一致確定）
**総計: 約694,000+パターン**

---

## v11→v12 変更履歴（2026/4/8-9）

- v6/案C WFA完了（TVトレードリスト直接分析方式）
- Pine→Python構造的不一致を確定（突合せ一致率23%、6層の差異を特定）
- build_nk225_database_v3.py作成（Panama Canal adjustment実装）→ 限月調整は相対指標に無影響と確認
- **方針転換: v6/案Cライブ執行をTV Webhook方式に変更決定**
- Gold EWMACは現行Pythonエンジン継続（Webhook対象外）
- 教訓#50-53追加
- Pine→Python移植リサーチレポート作成（6層の差異、業界事例、解決策3パターン）
- 教訓#54-59追加（次元独立性、エグジット遅延危険性、VM条件、AND条件固定、ADX位相分析）
- ADX位相分析スクリプト追加（scripts/research/: adx_phase_analyzer.py, adx_phase_cross_analysis.py, adx_feature_importance.py）

---

## v12→v13 変更履歴（2026/4/10）

### インフラ変更
- IBC config: IbLoginId=rhhane248（日本居住者はペーパーユーザー名で直接ログイン必須。kuri3desubot追加により旧方式が破綻）
- ペーパーアカウント: rhhane248 / DU8696858（kuri3desu/kuri3desubot共通）
- IB Key 2FA: rhhane248に対してIBKR Mobileアプリから直接登録（ペーパー側Client Portalには設定項目なし）
- Cloudflare Tunnel導入: webhook.torotakuno1.com → localhost:5001（Windowsサービスとして自動起動。ngrok不要に）
- ドメイン: torotakuno1.com（Cloudflare Registrar、年$10.46）

### 教訓
54. **日本居住者のIBKRペーパーアカウントは、ライブユーザー名+TradingMode=paperではログインできない。** ペーパー専用ユーザー名（rhhane248等）で直接ログインする必要がある。複数ユーザー追加時にGatewayが「multiple Paper Trading users」エラーを出す。
55. **Webhook受信にはトンネルサービスが必須。** ngrok無料は毎回URL変更で運用不可。Cloudflare Tunnel（無料+ドメイン年$10）が固定URL・自動起動・高安定性で最適。

---

## v13→v14 変更履歴（2026/4/10）

### Webhook E2Eテスト完了
- webhook_server.py clientId: 30→40に変更（fade_2h_engine.py clientId=30との衝突回避）
- start_trading.bat更新: v6_realtime_engine.py/caseC_realtime_engine.py削除、webhook_server.py/fade_2h_engine.py追加
- fade_2h_engine.pyのパスを修正（旧WS07パス→tv_data）
- E2Eテスト成功: curl→Cloudflare Tunnel→webhook_server→IBKR発注→Telegram通知

### clientId一覧（確定）
| clientId | エンジン |
|----------|----------|
| 3 | gold_ewmac_engine.py |
| 10 | v6_realtime_engine.py（廃止予定） |
| 20 | caseC_realtime_engine.py（廃止予定） |
| 30 | fade_2h_engine.py |
| 40 | webhook_server.py（v6+案C統合） |

### 教訓
56. clientIdは全エンジンで一意にすること。同一clientIdで2プロセスが接続するとIBKRが先行接続を切断する。新エンジン追加時は既存の割当表を必ず確認。

---

## v14→v15 変更履歴（2026/4/10）

### テスト結果
- v6/案CのES移植テスト: 両方とも機能せず（v6 RF 0.86、案C RF 0.31）。N225固有のエッジと確定
- Gold EWMACキャリー追加: コンタンゴ88%で逆方向。RF 6.78→3.10に悪化
- Gold EWMAC ADXフィルター: 7パターン全滅（同一次元）
- MNQ/MCL/M6E EWMAC 4年データ: 全滅（SR 0.064/-0.550/-0.050）
- pysystemtrade長期データ: 上記商品が数十年ではSR 0.3-0.5でプラス。4年データの結論は不十分

### インフラ
- pysystemtrade導入: WS07 C:\Users\CH07\pysystemtrade（253商品、長期データ付き）

### 教訓
- #60-63追加（上記参照）

---

## v15→v16 変更履歴（2026/4/12）

### フェード2H 最終構成確定
- エントリー: 日中2Hバー close < 直近20本安値 → ロングのみ
- F2: MACD(12,26,9) AND KVO(34,55) ブルクロス 10本以内
- F3: VR (VIX3M/VIX) > 1.00
- FOMC±1日除外
- エグジット: Hold 10回(slotConfirm単位) OR 1H MACD+KVO ベアクロス3本以内
- SLなし
- 成績: NP +259万 / DD 42.9万 / RF 6.05 / N=119 / PF 1.814 / WR 63%

### 検証経緯
- F1(ギャップ)12パターン → 全滅棄却
- F2(MACD+KVO)16パターン → #16(AND 10本)が最良
- F3(VR)7パターン → VR>1.00採用
- F4(カレンダー)8パターン → FOMC±1除外が最良、SQ週除外は逆効果
- Hold期間バグ発覚: slotConfirm単位のholdCountが実質5日保有。修正するとエッジ消失→スイング戦略として受容
- DMAフィルター4パターン → DMA2(LO)+FOMCがRF 4.39だがN=55で統計不足。フィルターなしLO(N=119)を採用
- エグジット21パターン+保護EX: 1H MACD+KVO逆クロスがRF 6.05で最良
- 保護EX時間足: 30m/45m/90m/2H全てが1Hに劣後（5TFテスト済み）
- SLシミュレーション: 200-1000pt全て逆効果
- v6/案Cとの補完性: 月次相関0.36、Fade独自保有日55%、ポートフォリオ+15%

### ポートフォリオ構成（4システム）
| Layer | システム | 商品 | TF | RF | 方式 |
|-------|----------|------|-----|------|------|
| 1 | v6 | N225マイクロ | 1H | 12.23 | TV Webhook |
| 1 | 案C | N225マイクロ | 1H | 8.77 | TV Webhook |
| 2 | Gold EWMAC | MGC | 2H | 17.14 | Pythonネイティブ |
| 3 | フェード2H | N225ミニ | 1H(2Hスロット) | 6.05 | TV Webhook(実装予定) |

### 教訓
- #64-67追加（上記参照）

## v16→v17 変更履歴（2026/4/14）

### N225マイクロ 日中戦略スクリーニング → コスト構造上不成立と確定

1分足データ（2013-2026、360万本）を使用し、8戦略カテゴリ×複数バリアントを中立コストモデルで検証。全てRF<0。

**中立コストモデル（以降の全Pythonバックテストの標準）:**
- エントリースリッページ: 5pt（1ティック）
- ストップスリッページ: 5pt（1ティック）
- イグジットスリッページ: 0pt（指値想定）
- 手数料: 76円/往復
- 同一バーH/L両ヒット: open距離で方向判定（保守版はスキップだが過剰）

**検証結果サマリー:**

| 戦略 | RF(中立コスト) | PF | N | 備考 |
|------|----------------|------|------|------|
| 石原順(BB+ADX+StdDev) | -1.00 | <0.75 | 4,211-37,860 | 48パターン(σ0.3-2.5×フィルター)全滅 |
| CO-OC反転 | -0.54〜0.19 | 0.95-1.02 | 1,321-2,477 | 4バリアント。PF≒1.0(コイン投げ) |
| ORB5朝 | -1.00 | 0.20 | 3,232 | コストなしRF13.52→中立で消滅。25フィルターも全滅 |
| ORB15/30朝 | -1.00 | 0.16-0.17 | 3,165-3,221 | ORB5と同様 |
| 日中モメンタム(朝30m→引け30m) | -0.99 | 0.48 | 3,138 | 14/14年赤字 |
| MACD(4,22,3) 5分足 | -1.00 | 0.34 | 40,604 | Kang論文N225最適パラメータ |
| RSI(2) 15分足 | -0.92 | 0.74 | 2,235 | 「最善」だがRF<0 |
| TTMスクイーズ 5分足 | -0.94 | 0.80 | 3,042 | 11/14年赤字 |

**ORB5 Pine Script検証:**
- TV 5分足チャート: RF 3.00, NP +164万(ミニ), 3,565トレード, 2011-2025
- Python 1分足(コストなし): RF 13.52 → Pythonが4.5倍過大評価
- WFA(IS2年/OOS1年): OOS黒字率83%(10/12窓), OOS連結RF 5.17, ISパラメータ不安定(OR30が5回/12窓選択)
- 中立コスト適用後: RF -1.00 → 全フィルター(MACD4種+曜日+SQ+NR7+OR幅+ギャップ+前日レンジ)も全て-1.00

**出来高分析:**
- 二重U字型確認: 9時ピーク(4,371/5分), 11:55ボトム(480), 14:55回復(2,100)
- Volume Spike方向予測: 42-43%（ランダム以下=逆方向に動く傾向）
- 簡易Tick Imbalance(1分足 方向×出来高): 41.2%（ランダム以下）
- 年別: 全年40%前後で安定して「逆」

**fade_2hとの関係確認:**
- CO-OC裸: RF 0.15 vs fade_2h: RF 6.05 → fade_2hのエッジはCO-OC反転ではない（MACD+KVOが独自エッジ）
- fade_2hとORB5の相関: ほぼゼロ（メカニズム/時間軸/頻度が全て異なる）。ただしORBはコスト込みで不成立のため実用不可

### OFI（板データ）調査 — 中断中

OHLCV出来高の方向予測力がゼロだったが、これは真のOFI（板データ）の否定にはならない。約定データの逆方向傾向は「指値側（板）に情報がある」ことの間接証拠とも読める。

**未完了の調査2件:**
1. J-Quants DataCubeティックデータ（¥330/月）のカラム構成確認 — 買い/売り区分・気配値の有無で簡易OFI計算可能か判断
2. au kabucom Kabu Station API の板データ仕様 — リアルタイムOSE板のプログラマティックアクセス可否

**IBKRのOSE APIブロック（確認済み）:**
- TWS APIはOSEのリアルタイムマーケットデータを配信しない（15分遅延のみ）
- OSE L2サブスク(¥400/月)に課金してもAPI経由ではライブ不可
- TWS画面上（BookTrader等）ではライブ板が見える
- CME NKD/NIYはAPIフルサポート（回避策の一つ）

### 教訓

68. **N225マイクロの5-15分足日中完結戦略は手数料構造上エッジ維持不能。** ティック値(50円)/手数料(76円)=比率0.7:1。1回あたり固定コスト126円(滑り50+手数料76)を超える平均利益(12.6pt)が日中短期では達成困難。8戦略カテゴリ全滅で確定。比較: ES(1tick=$12.50/手数料$2.25=5.6:1), N225ミニ(1tick=500円/手数料~200円=2.5:1)。

69. **Pythonバックテストは中立コストモデルを標準とすべき。** コストなしPython(RF13.52)とTV実測(RF3.00)の乖離4.5倍。fade_2hでも同パターン（Python良すぎ→Pine現実的）。entry_slip=5pt, stop_slip=5pt, 手数料76円RTを全Pythonバックテストのデフォルトにすること。

70. **CO-OC反転はN225で構造的に機能しない。** ナイトセッションがオーバーナイトリターンを先取りするため、翌朝ギャップの反転エッジが消滅。学術論文のSR4.0超は米国指数（ナイトセッションなし）の結果。

71. **fade_2hのエッジはCO-OC反転ではない。** CO-OC裸RF0.15 vs fade_2h RF6.05（60倍差）。MACD+KVOの指標組み合わせが独自のエッジを構成していることが数値で裏付けられた。指標は冗長ではない。

72. **OHLCV出来高ベースの方向予測力はN225でゼロ（41-43%）。** ただしOHLCV出来高がダメ≠真のOFI（板データ）がダメ。約定データの逆方向傾向は指値側に情報があることの間接証拠とも読める。

73. **石原順モデル(BB+ADX+StdDev)はN225マイクロ全パラメータでエッジなし。** σ0.3-2.5の12段階×フィルター有無×2TF=48パターンでPF最大0.75。コアロジックにエッジがない。

### スクリプト（WS07: C:\Users\CH07\Desktop\jquants_data\に存在。全滅のためリポジトリにはpushしない）
- co_oc_reversal_backtest.py, orb_nr7_backtest.py, orb_wfa.py
- orb5_filter_v2.py(保守), orb5_filter_v3.py(中立)
- strategy_screening.py, volume_analysis.py
- 石原順: ishihara_backtest_local.py, ishihara_backtest_v2.py, ishihara_sigma_sweep.py

## v16→v17 変更履歴（2026/4/14）

### 日中戦略スクリーニング — 全滅確定
- 石原順ボラティリティモデル: 48パターン全てRF<0
- CO-OC反転: 4バリアント全てPF≒1.0（RF最良0.15）
- ORB+NR7: コストなしRF=13.52 → 中立コスト(entry5pt/stop5pt/手数料76円)でRF=-1.00
- 6戦略一括(MACD/RSI/TTM/ORB15/ORB30/日中モメンタム): 中立コスト全滅
- 出来高方向予測: 42-43%（ランダム以下）
- **結論: N225マイクロのティック値(50円)/手数料(76円)=0.7:1。日中高頻度は構造的に不成立**

### OFI（Order Flow Imbalance）調査 — 棄却
- J-Quants DataCube先物歩み値(ティック)カラム確認: 売買区分なし。約定データのみ（Trade_Date/Time/Trade_Price/Price_Type/Trade_Volume/No/Contract_Month + sco_category）
- N225mini 2025年11月データ（335万行/18営業日）でTick Rule OFIプロトタイプ検証
- 結果: 全タイムフレーム（1分/2分/3分/5分/15分/1H）で予測一致率≒50%。予測相関≒0。方向予測力ゼロ
- 強OFI(|z|>1): 52.3%、極強OFI(|z|>1.5): 45.8%（逆方向）
- 同時一致率は76-77%で安定 → OFIは後追い指標（同一バー内で価格に織り込み済み）
- au kabucom(三菱UFJ eスマート証券)板データAPI調査: 口座無料だがOFI死亡のため口座開設不要と判断
- **結論: Tick Rule OFI × 分足以上 × 方向フィルターは構造的に不成立。板データ真OFIもms単位の執行最適化用であり、1H足フィルターには使えない**

### スクリプト（WS07: C:\Users\CH07\Desktop\jquants_data\）
- co_oc_reversal_backtest.py, orb_nr7_backtest.py, orb_wfa.py
- orb5_filter_v2.py(保守), orb5_filter_v3.py(中立)
- strategy_screening.py, volume_analysis.py
- ofi_prototype.py（Tick Rule OFI検証）

### 教訓

68. **N225マイクロの日中高頻度戦略はコスト構造上不成立。** ティック値(50円)/手数料(76円)=比率0.7:1。ORB(中立コストRF=-1.00)、MACD、RSI、TTM等すべて中立コストで全滅。有効戦略は1H足以上の低頻度に限定。比較: ES(1tick=$12.50/手数料$2.25=5.6:1)。

69. **Pythonバックテストではエントリーを常にopen[i+1]で計算すべき。** PineのデフォルトはバーN条件成立→バーN+1始値で約定。Pythonでclose[i]エントリーとすると未来参照（look-ahead bias）となり成績を過大評価する。ORBでPython RF=13.52 vs Pine RF=3.00（4.5倍乖離）の主因の一つ。中立コストモデル（entry5pt/stop5pt/手数料76円RT）と合わせてPythonバックテストの標準とすること。

70. **CO-OC反転はN225で構造的に機能しない。** ナイトセッションがオーバーナイトリターンを先取りしている。学術論文のSR4.0超は米国指数（ナイトセッションなし）の結果。N225ではCO-OC裸RF=0.15。fade_2hのエッジはCO-OC反転ではなくMACD+KVOの指標組み合わせが独自に構成（CO-OC RF=0.15 vs fade_2h RF=6.05）。

71. **Tick Rule OFIはN225miniで方向予測力ゼロ。** 1分～1H全タイムフレームで予測一致率≒50%（N=335万ティック、2025年11月）。同時一致率77%は「OFIが既に起きた値動きを測っている」だけの後追い指標。OFIの予測力はミリ秒～秒の世界に限定され、分～時間では消失する。1H足フィルターとしてのOFI活用は構造的に不成立。

72. **OFIで「方向予測力ゼロ」はデータ品質の問題ではなくN225miniの効率性の問題。** 流動性が十分な市場では注文流情報が同一バー内でリアルタイムに価格反映される。板データ（真のOFI）でもms単位の執行最適化には使えるが、分足以上の方向フィルターとしては構造的に効かない見通し。プロのOFI活用は「方向予測」ではなく「気配値調整・在庫管理・スプレッド制御」。

73. **個人自動売買の有効時間足は1H以上。** v6(1H/RF12.23)、案C(1H/RF8.77)、Gold EWMAC(2H/RF17.14)、Fade 2H(実質数日/RF6.05)は全て1H以上で機能。5分ORB(RF-1.00)、1分MACD(RF-1.00)、15分RSI(RF-0.92)、ティックOFI(予測力ゼロ)は全て1H未満で全滅。境界線は明確で、高頻度はコスト比率と情報即時反映の両方に殺される。

## v17→v18 変更履歴（2026/4/15）

### Gold EWMAC v4.0 監査修正 — ポジションサイズ過大バグ修正

gold_ewmac_engine.py v3.1 のコード監査で3つのポジションサイズ計算バグを発見・修正。

**バグ詳細と修正:**

| # | 箇所 | v3.1 (旧) | v4.0 (修正) | 影響 |
|---|------|-----------|-------------|------|
| 1 | CONFIG fdm | 1.3 (4var用) | 1.04 (2var, Carver Table 48.1) | ポジション25%過大 |
| 2 | _calc_forecast ideal_position | fdm二重適用 | 削除（combinedに含有済み） | ポジション30%過大 |
| 3 | CONFIG ann_factor | 1792 | 3080 (実測2Hバー/年) | ポジション31%過大 |

**複合効果:** v3.1のポジションサイズはv4.0比で約2.1倍過大。

**バックテスト比較（2Hバー直接、2019-2026）:**

| 指標 | v3.1 (旧) | v4.0 (修正) | 変化 |
|------|-----------|-------------|------|
| NP (万) | +633.9 | +390.8 | -38% |
| DD (万) | 81.2 | 46.0 | -43% |
| RF | 7.81 | 8.49 | +8.7% |
| 平均枚数 | 0.85 | 0.60 | -29% |

**追加修正:** ウォッチドッグ6h→3h、再購読前cancelHistoricalData()追加(Error 10182対策)

**注意:** 旧RF 17.14は日足resample+旧CONFIG(4var/FDM1.3/capital200万)の値。v4.0のRF 8.49が正しい値。

### 教訓

74. **バックテストのTimeframeと本番エンジンのTimeframeを一致させること。** ann_factorは実データのbars_per_yearから計測。Carverの公式: annual_vol = bar_vol x sqrt(bars_per_year)。

75. **FDMはバリエーション数に応じた正しい値を使うこと。** Carver Table 48.1: 2var→1.03-1.12, 4var→1.3。forecast結合時とposition sizing時の二重適用に注意。
## v18→v19 変更履歴（2026/4/14-15）

### 原油(CL) 1Hクラス大規模スクリーニング — 3候補特定、Pine Script検証待ち

#### 概要
CLラージ先物（$0.0025/barrel）を前提に、30m/1H/2Hの3時間足で大規模指標スクリーニングを実施。
v3（38シグナル）→ v4（62シグナル、1,722有効ペア）で約63万パターンのエントリー×出口を網羅テスト。
その後、上位3エントリーに対して14カテゴリの出口・フィルターを徹底検証（合計数千パターン追加）。

**コスト前提:** CLラージ（CL, 1000バレル）。MCLマイクロ（100バレル）ではコスト比率が10倍悪化しエッジ消滅（v1/v2で確認済み）。CLラージの証拠金は約$6,000（≒90万円）。

#### v4スクリーニング結果（上位3エントリー）

| TF | エントリー | 出口 | RF | PF | WR | N | 備考 |
|---|---|---|---|---|---|---|---|
| 30m | WTI-Brentスプレッド > 20MA | hold_40 | 11.61 | 1.694 | 56.2% | 571 | WS07データ(2年) |
| 1H | Aroon(14)+KVO(20,40) OR | hold_40 | 8.78 | 1.513 | 55.3% | 483 | WS07データ(2年) |
| 2H | Ichimoku雲+StdDev(20,1.0) OR | hold_20 | 9.14 | 1.369 | 53.2% | 755 | Macデータ(7年) |

※ 30m/1HのRFはWS07データ(23,606バー)基準。Macデータ(13,746バー)ではRFが異なる（データ期間差）。

#### v4で追加した25新シグナル
Ichimoku雲ブレイク / Supertrend(10/2,10/3,7/3) / Donchian(20,55) / PSAR / HMA(20,40) / CCI(14,20) / Stochastic(14,3) / Williams%R(14) / ROC(10,20) / TSI(25,13) / Aroon(14,25) / Vortex(14,21) / CMF(20) / MFI(14) / ADX+DI方向(14/25,20/20)

v4新規シグナルからの貢献: Ichimoku（2H 3位）、Aroon（1H 2位）、CCI（30m上位）

#### 出口・フィルター網羅テスト結果（14カテゴリ）

| # | テスト | 30m | 1H | 2H | パターン数 |
|---|---|---|---|---|---|
| 1 | ATR SL/TP/SL+TP（v4内） | ❌ | ❌ | ❌ | v4に含む |
| 2 | トレーリング（v4内） | ❌ | ❌ | ❌ | v4に含む |
| 3 | 反転シグナル（v4内） | ❌ | ❌ | ❌ | v4に含む |
| 4 | VM消灯（σ×10種） | ❌ | ❌ | ❌ | 840 |
| 5 | ADX上昇フィルター（即時+遅延許容） | ❌ | ❌ | ❌ | 99 |
| 6 | 遅行線(Chikou Span)利確 | ❌ | ❌ | ❌ | 78 |
| 7 | ボラ適応型保有期間（ATR比率） | ❌ | ❌ | ❌ | 162 |
| 8 | MTF方向フィルター（2H→30m/1H） | ❌ | ❌ | — | 42 |
| 9 | 日足MA傾き | ❌ | ❌ | ❌ | 93 |
| 10 | **ゲート付きシャンデリア** | △+0.19 | **✅+2.64** | ❌ | 1,245 |
| 11 | SL安全弁（ゲート付き+SL） | — | ❌ | — | 254 |
| 12 | ADX/StdDevピークアウト出口 | — | △-0.61 | — | 817 |
| 13 | カレンダー・アノマリー（曜日/セッション/月/ロール） | ❌過適合 | ❌ | ❌ | 114+60 |
| 14 | OVX+タームストラクチャ | 要確認 | ❌ | ❌ | 117 |

#### ゲート付きシャンデリア出口（唯一の改善）

1H Aroon+KVO ORエントリーで `gChand_g25_a3.0_lb14_c1_cap60` がhold_40を上回った。

| 出口 | RF | PF | DD | N |
|---|---|---|---|---|
| hold_40（ベースライン） | 8.78 | 1.513 | 20.8 | 483 |
| gChand(g25/a3.0/lb14/c1/cap60) | **11.42** | **1.643** | **13.5** | 280 |

構造: 最初25本は出口無視 → 25本後にシャンデリア起動（14本高値/安値 - 3.0×ATR） → 60本で強制手仕舞い。SLなし（SL追加は全て悪化、教訓#76参照）。

#### ロール除外フィルター — 過適合と判定

30mで固定日「月の15-20日」除外がRF 19.16と突出したが、以下の理由で過適合と判定:
1. 実際のCLロール日（納品月25日の3営業日前）と固定「15-20日」は一致しない
2. 実ロール日ベースのフィルター（RF 10.56）より固定日（RF 19.16）の方が良い = ロール構造と無関係
3. 2Hの7年データでは固定日も実ロール日も不成立
4. RF改善の主因がDD縮小（特定損失の偶然回避）

#### OVXフィルター — 要追加検証

30mで `OVX_above_30` がRF 6.05→10.02（+66%）に改善。ただし:
- 2Hの7年データでは+0.21の誤差レベル
- Gold EWMACのGVZ zscore方向フィルターとは構造が異なる（レベル vs 方向）
- 30mデータが2年と短い
- **TV上でのPine Script長期検証が必要**

#### 確定構成（Pine Script検証待ち）

| TF | エントリー | 出口 | フィルター |
|---|---|---|---|
| 30m | WTIBrent_20 | hold_40 | なし（OVX>30は要確認） |
| 1H | Aroon(14)+KVO(20,40) OR | gChand(g25/a3.0/lb14/c1/cap60) | なし |
| 2H | Ichimoku雲+StdDev(20,1.0) OR | hold_20 | なし |

### 教訓

76. **原油ではテクニカル出口・フィルター積層が構造的に不成立。** 14カテゴリ（ATR SL/TP、トレーリング、VM消灯、ADXフィルター、遅行線利確、ボラ適応型保有期間、MTF方向フィルター、日足MA傾き、カレンダー、ピークアウト出口、OVX、タームストラクチャ）を網羅テストし、hold_40を安定的に超えたのはゲート付きシャンデリア(1Hのみ)だけ。N225のフィルター積層型（VM+NTスコア+MTF+日足方向+傾き+LDN16+SQ週）は商品固有であり、原油には適用不可。

77. **テクニカル出口が固定バー出口に負ける理由は「取引回数爆増」。** VM消灯、遅行線利確、ADXフィルター等は全てN 571→3,000-10,000に取引回数が爆増し、コスト負けする。学術研究（Clare et al. 2012, Kaminski & Lo 2014）も「モメンタム戦略にSLを追加すると期待リターンが数学的に下がる」と結論。Kevin Davey 56.7万回バックテストでもSL+TPの組み合わせが最悪。

78. **ゲート付き出口が唯一機能する理由は「ノイズ期間の無視」。** 最初N本間を出口無視にすることで、エントリー直後のノイズ反応→早期手仕舞い→再エントリーのコスト負けサイクルを断ち切る。ゲートなしのシャンデリアやトレーリングは同じ罠にハマる。

79. **SL安全弁はゲート付きシャンデリアでも逆効果。** SLなしRF 7.99 > SL10×ATR RF 6.87 > SL3×ATR RF 2.59。「回復可能なトレードを損失確定する」コストが「壊滅的損失を回避する」便益を上回る。リスク管理はSLではなくポジションサイズ（VT）で行うべき。

80. **カレンダーフィルターの過適合判定には長期データと実日程の両方が必要。** 固定日フィルターが実日程フィルターより良い結果を出す場合、それは構造的効果ではなく期間固有の偶然。CLロール除外（固定日RF 19.16 > 実ロール日RF 10.56）で確認。

81. **OVXレベルフィルターはGVZ zscoreフィルターとは異なる構造。** Gold EWMACのGVZ zscore>0（方向フィルター）はOOS検証で有効性確認済み。CL30mのOVX>30（レベルフィルター）は2年データでのみ有効（2H 7年では誤差）。Pine Script上での長期検証が必要。

82. **CLラージとMCLマイクロのコスト構造差は10倍。** MCLマイクロ: tick=$0.01×100=$1, 手数料≒$0.65 RT → コスト比率低い。CL: tick=$0.01×1000=$10, 手数料≒$2.50 RT → コスト比率高い。MCLでは全滅したエントリーがCLで成立する。商品選定時はコスト構造を最初に確認すること。

83. **「ルールを増やすより商品を増やせ」はCarverの原則だが、商品固有の1H足開発にも価値がある。** pysystemtrade日足EWMACでCL SR=0.120（低い）だが、1H足の指標系シグナルでRF 8-11が出る。N225がその先行事例（日足EWMAC SR低いが1H v6 RF 12.23）。

### スクリプト所在

**WS07: C:\Users\CH07\nikkei-trade\scripts\research\**
- cl_combo_v3.py — v3スクリーニング（38シグナル、CLラージコスト、出口30種）
- cl_combo_v4.py — v4スクリーニング（62シグナル、63万パターン）
- cl_v3_results.csv / cl_v4_results.csv — 結果
- cl_vm_exit_results.csv — VM出口テスト結果
- cl_adx_filter_results.csv — ADXフィルター結果
- cl_advanced_exit_results.csv — ゲート付きシャンデリア+ボラ適応型結果
- cl_dma_slope_results.csv — 日足MA傾き結果

**Mac: ~/Downloads/**
- cl_gated_chand_sl_results.csv — ゲート付きシャンデリア+SL安全弁
- cl_peakout_exit_results.csv — ピークアウト出口
- cl_chikou_exit_results.csv — 遅行線利確
- cl_calendar_filter_results.csv — カレンダー・アノマリー
- cl_actual_roll_results.csv — 実ロール日フィルター
- cl_ovx_term_results.csv — OVX+タームストラクチャ

### 次のアクション

1. **【最優先】上位3エントリーをPine Script化 → TV strategy testerで長期検証**
   - 30m WTIBrent_20 + hold_40
   - 1H Aroon(14)+KVO(20,40) OR + gChand(g25/a3.0/lb14/c1/cap60)
   - 2H Ichimoku雲+StdDev(20,1.0) OR + hold_20
2. **OVX>30フィルターをPine Script上で長期検証**（30mの2年データでは有望だが確定には至らない）
3. TV検証でRF>3維持 → WFA実施 → ポートフォリオ組入れ検討
4. TV検証でRF<1 → 原油見送り → 別商品（ES/ZN）へ移行
5. CLラージの証拠金（~90万円/枚）が300万口座の30%を占有する点を考慮したポジションサイジング設計


## v19→v20 変更履歴（2026/4/18）

### Gold EWMAC ウォッチドッグ Telegram通知抑制

#### 背景

gold_ewmac_engine.py v4.0 のウォッチドッグが3時間おきに「ウォッチドッグ発動: 3.0h バー無し」→「ストリーミング再購読OK」の通知ペアをTelegramに送信していた。これはMGC (COMEX Gold) の取引時間ギャップで一定時間バーが来ない際にib_insyncの`reqHistoricalData(keepUpToDate=True)`が内部的にストリームを切り、ウォッチドッグが自動検知→再購読する**設計通りの自己修復動作**。

問題は通知頻度。1日8回の通知ペア（計16メッセージ/日）が発火することで、エントリー/決済/切断などの**真に行動が必要な通知を見落とすリスク**が発生していた。

#### 変更内容

**ファイル:** `scripts/execution/gold_ewmac_engine.py`

**削除行:**
```python
# self.tg.warn("Gold EWMAC", f"ウォッチドッグ発動: {CONFIG['watchdog_timeout_hours']}h バー無し")
# self.tg.send("Gold EWMAC ストリーミング再購読OK")
```

**残した行:**
- `log.warning("ストリーミング再購読...")` — ログ記録
- `log.info("ストリーミング再購読OK")` — ログ記録
- `self.tg.error(...)` 再購読失敗時 — 真の異常通知

**コミット:** `9103fe1` "Sync gold_ewmac v3.1 -> v4.0 (audit fixes + watchdog 3h) and suppress watchdog telegram notifications"

### 副次効果: gitリポ↔実行フォルダのv4.0乖離解消

v17→v18変更時点で、Gold EWMAC v4.0の修正（FDM=1.04 / ann_factor=3080 / fdm二重適用削除 / ウォッチドッグ6h→3h / cancelHistoricalData追加）は**実行フォルダ（`C:\Users\Riku\Desktop\tv_data\`）のみに反映**され、gitリポ（`C:\nikkei-trade\scripts\execution\`）には反映されていなかった。

今回の作業で以下を同時に解消:
- 実行フォルダのv4.0をgitリポへコピー
- ウォッチドッグ通知抑制を適用
- 単一コミット `9103fe1` として統合してpush

### Fade 2H Webhook移行完了

v16時点で「Fade 2H: TV Webhook(実装予定)」だった構成が、v14時点のwebhook_server導入を経て、**Pythonネイティブ実装(fade_2h_engine.py)からTV Webhook方式に移行完了**。

**現在のポートフォリオ構成（確定）:**

| Layer | システム | 商品 | TF | RF | 執行方式 | 稼働プロセス |
|-------|----------|------|-----|------|----------|--------------|
| 1 | v6 | N225マイクロ | 1H | 12.23 | TV Webhook | webhook_server.py (clientId=40) |
| 1 | 案C | N225マイクロ | 1H | 8.77 | TV Webhook | webhook_server.py (clientId=40) |
| 2 | Gold EWMAC | MGC | 2H | 8.49 | Pythonネイティブ | gold_ewmac_engine.py (clientId=3) |
| 3 | Fade 2H | N225ミニ | 1H(2Hスロット) | 6.05 | TV Webhook | webhook_server.py (clientId=40) |

**miniPC稼働プロセス（2本のみ）:**
- `webhook_server.py` (clientId=40) — v6/案C/Fade 2Hの3システム統合受信
- `gold_ewmac_engine.py` (clientId=3)

### インフラ観測事項

**wmicコマンドWin11非対応化:**
IBCログ（`C:\IBC\Logs\IBC-3.23.0_GATEWAY-1037_.txt`）に以下のエラーが出現:
```
'wmic' は、内部コマンドまたは外部コマンド、操作可能なプログラムまたはバッチファイルとして認識されていません。
```

Windows 11 22H2以降、wmicは「機能オプション」扱いとなりデフォルト非インストールの環境がある。現時点で実害なし（IBC起動は成功、Gold EWMAC/webhook_serverも正常接続）。プロセス監視コマンドとして将来的に以下への置き換え検討:
- PowerShell: `Get-CimInstance -ClassName Win32_Process`
- CMD代替: `tasklist /FI "IMAGENAME eq python.exe" /V`

### 教訓

84. **自己修復動作のTelegram通知はノイズ化する。** 再購読・再接続が即座に成功する正常動作は**Telegram通知する価値がない**。「行動が必要な事象のみTelegram、状態記録はログ」の原則を徹底すること。通知が多すぎると真の警告（切断持続、発注失敗、ポジション不一致）を見落とす運用疲労が最大のリスク。

85. **gitリポと実行フォルダの乖離は定期的に検出すべき。** 実行フォルダ直接編集→gitコピー忘れのパターンが発生しうる。監査時は以下のコマンドで差分確認:
    ```cmd
    fc C:\nikkei-trade\scripts\execution\<file>.py C:\Users\Riku\Desktop\tv_data\<file>.py
    ```
    差分があれば実行フォルダ側を正としてリポへ反映し、即commit。複数の変更が蓄積する前に同期する運用が安全。

86. **通知の「行動必要性」で分類すること。** 通知の重要度を以下3レベルで判定:
    - **必須通知（Telegram常時）:** エントリー、決済、切断持続、再接続失敗、ポジション不一致、起動、手動停止
    - **削除候補（ログのみ）:** 自己修復動作（ウォッチドッグ発動→即再購読OK、定期接続確認）
    - **要判断（連続発生時のみTelegram）:** 再購読の連続失敗、間欠的切断

### 次のアクション

1. **他エンジンの通知棚卸し（優先度中）** — `webhook_server.py`, `fade_2h_engine.py`の再接続通知も同基準（教訓#86）で点検。ノイズ通知があれば同様に抑制。
2. **wmicコマンドの代替整備（優先度低）** — 現状実害なし。将来IBCアップデートやWindowsアップデート時に問題が顕在化したら対応。
3. **動作確認（自動進行）** — 2026/4/18 14:40起動のGold EWMACで、ウォッチドッグ発動タイミング（起動後約3時間ごと）にTelegram通知が来ないことをログで確認。`logs/gold_ewmac_20260418.log`に"ウォッチドッグ発動"の記録があり、かつTelegramに到達していなければ成功確定。
