#!/usr/bin/env python3
"""
Gold EWMAC Engine v4.0 — 監査修正版
======================================
v3.1からの変更点:
  [#1] FDM: 1.3 → 1.04 (Carver Table 48.1, 2 variations)
  [#2] FDM二重適用バグ修正: _calc_forecast内のideal_positionから cfg['fdm'] 削除
  [#3] ann_factor: 1792 → 3080 (実測2Hバー/年)
  [#4] ウォッチドッグ閾値: 6h → 3h (2Hバー1.5本分)
  [#5] 再購読前に既存ストリーミングを明示的にキャンセル (Error 10182対策)
  [#6] _resubscribe()メソッド追加 (run()ループ内ウォッチドッグ統合)

使い方:
  python gold_ewmac_engine.py          # ペーパー(4002)
  python gold_ewmac_engine.py --live   # 本番(4001)
"""
import sys, time, json, logging, urllib.request, ssl
...
# ============================================================
# GVZ Filter (unchanged from v3.1)
# ============================================================
class GVZFilter:
    ...
    def update(self):
        try:
            ...
            with urllib.request.urlopen(req, timeout=15, context=ctx) as resp: data = json.loads(resp.read())
            observations = data.get('observations', [])
            if not observations: log.warning("FRED: GVZデータなし"); return False
            values = []
            for obs in reversed(observations):
                if obs['value'] != '.':
                    values.append({'date': obs['date'], 'close': float(obs['value'])})
...
class EWMACEngine:
    ...
    def _calc_forecast(self, close):
        ...
        # [#2] FDM二重適用修正: combined に既に fdm が含まれているため、ここでは fdm を掛けない
        self.ideal_position = np.clip(
            (self.cfg['capital'] * self.cfg['vol_target'] * self.cfg['idm'] * w * self.current_forecast)
...
# ============================================================
# Realtime Engine — [#4][#5][#6] ウォッチドッグ改善
# ============================================================
class GoldEWMACRealtimeEngine:
    def __init__(self, is_live=False):
        ...
        self._last_bar_received = None  # [#4] ウォッチドッグ用タイムスタンプ
        self.live = None  # [#5] ストリーミング参照保持

    def connect(self):
        mode = 'LIVE' if self.is_live else 'Paper'
        log.info("=" * 50)
        log.info(f"=== Gold EWMAC v4.0 === {mode} port:{self.port} {CONFIG['capital']/10000:.0f}万 Max{CONFIG['max_contracts']}")
        log.info(f"    FDM={CONFIG['fdm']} ann={CONFIG['ann_factor']} IDM={CONFIG['idm']} n_inst={CONFIG['n_instruments']}")
        log.info("=" * 50)
    ...
    def _subscribe_bars(self):
        """[#5] ストリーミングバー購読。既存があればキャンセルしてから再購読"""
        if self.live is not None:
            try:
                ...
                log.warning(f"ストリーミングキャンセル失敗(無視): {e}")
        ...
        self._last_bar_received = datetime.now()
        self._pending = None
        log.info("ストリーミング購読開始")

    def _resubscribe(self):
        """[#5][#6] ストリーミング再購読（ウォッチドッグ or Error 10182 からの復帰）"""
        log.warning("ストリーミング再購読...")
        try:
            self._subscribe_bars()
            log.info("ストリーミング再購読OK")
        except Exception as e:
            log.error(f"再購読失敗: {e}")
            self.tg.error("Gold EWMAC", f"再購読失敗: {e} 再接続予約")
    ...
    def _on_bar(self, bars, has_new):
        ...
        self._last_bar_received = datetime.now()  # [#4] ウォッチドッグ更新
    ...
    def _reconn(self):
        try:
            ...
            self._subscribe_bars()  # [#5] 統合メソッド使用
            self.ib.disconnectedEvent += self._on_disc
            self._needs_reconnect = False; log.info("復元完了"); return True
        except Exception as e:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # [#4][#6] ウォッチドッグ: バー未受信が閾値超過で再購読
                if (self._last_bar_received is not None and
                    (datetime.now() - self._last_bar_received).total_seconds()
                    > CONFIG['watchdog_timeout_hours'] * 3600):
                    self._resubscribe()
                self.ib.sleep(1)
