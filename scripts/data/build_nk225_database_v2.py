"""
日経225ミニ先物 1分足データベース構築スクリプト v2
J-Quants DataCube → 連続先物 → 各タイムフレーム集約

v1→v2 変更点:
  1. バー境界をTV NK225M1!と一致させるカスタム1H集約
     - ～2024/11/4: 夜間:30始まり, 日中:30始まり(初回08:45)
     - 2024/11/5～: 夜間:00始まり, 日中:00始まり(初回08:45)
  2. ロール処理: SQ 7日前 → SQ当日(第2金曜)に変更
     TVのNK225M1!はSQ当日にロールすることを実データで確認済み
  3. 5分足のresampleもセッション境界対応

使い方:
  python build_nk225_database_v2.py /path/to/zip_folder
...
# OSE取引時間変更日 (2024/11/5 ～)
OSE_TIME_CHANGE_DATE = datetime(2024, 11, 5)
...
def assign_tv_1h_bucket_fast(dt_series):
    ...
    1分足数百万行に対応するためnumpy/pandasの条件マスクを使用。
    """
    n = len(dt_series)
    ...
    is_new = dt_series >= OSE_TIME_CHANGE_DATE
    ...
    mask = old & (((h == 8) & (m >= 45)) | ((h == 9) & (m < 30)))
    result[mask] = d[mask] + pd.Timedelta(hours=8, minutes=45)

    # 日中: 09:30-15:29 → :30始まり
    mask = old & (h >= 9) & ~((h == 9) & (m < 30))
    # 09:30-09:59 → 09:30, 10:00-10:29 → 09:30, 10:30-10:59 → 10:30, ...
    # floor to :30: if m>=30 → h:30, else → (h-1):30
    bucket_h = h[mask].copy()
    bucket_m_offset = pd.Series(30, index=bucket_h.index)
    # m < 30 → previous hour's :30
    sub_mask = m[mask] < 30
    bucket_h[sub_mask] = bucket_h[sub_mask] - 1
    result[mask] = d[mask] + pd.to_timedelta(bucket_h, unit='h') + pd.Timedelta(minutes=30)

    # 夜間: 16:30-17:29 → :30始まり
    mask = old & (h == 16) & (m >= 30)
...
def main():
    ...
    log_lines = []
    log(f"{'='*60}", log_lines)
    log(f"日経225ミニ 連続先物データベース構築 v2", log_lines)
    log(f"  バー境界: TVバー境界一致", log_lines)
